...
const NOT_FOUND: CompanyResolve = { exists: false, profileId: null, profileSubdomain: null, hasCms: false }
...
export async function resolveCompany(slug: string, now: number = Date.now()): Promise<CompanyResolve> {
  const hit = cache.get(slug)
  if (hit && hit.expires > now) return hit.value
  try {
    const res = await fetch(`${backendBase()}/public/companies/resolve/${encodeURIComponent(slug)}`, {
      cache: 'no-store',
    })
