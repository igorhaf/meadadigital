...
export function listClasses(opts: { onlyActive?: boolean } = {}): Promise<{ items: EscolaClass[] }> {
...
export function createClass(input: CreateClassInput): Promise<EscolaClass> {
  return apiFetch<EscolaClass>('/api/escola/classes', { method: 'POST', body: JSON.stringify(input) })
}

export function updateClass(id: string, input: UpdateClassInput): Promise<EscolaClass> {
  return apiFetch<EscolaClass>(`/api/escola/classes/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}

export function toggleClass(id: string, active: boolean): Promise<EscolaClass> {
  return apiFetch<EscolaClass>(`/api/escola/classes/${id}/toggle`, { method: 'PATCH', body: JSON.stringify({ active }) })
}
