...
import type { Order, OrderStatus } from '@/profiles/las/las-types'
...
export function listOrders(
  ...
  const qs = p.toString()
  return apiFetch<OrderPage>(`/api/las/orders${qs ? `?${qs}` : ''}`)
}

export function getOrder(id: string): Promise<Order> {
  return apiFetch<Order>(`/api/las/orders/${id}`)
}
...
export function updateOrderStatus(
  ...
  return apiFetch<Order>(`/api/las/orders/${id}/status`, {
    method: 'PATCH',
