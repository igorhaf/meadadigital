...
export function createZone(input: CreateZoneInput): Promise<ComidaDeliveryZone> {
  return apiFetch<ComidaDeliveryZone>('/api/comida/zones', { method: 'POST', body: JSON.stringify(input) })
}

export function updateZone(id: string, input: UpdateZoneInput): Promise<ComidaDeliveryZone> {
  return apiFetch<ComidaDeliveryZone>(`/api/comida/zones/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}
