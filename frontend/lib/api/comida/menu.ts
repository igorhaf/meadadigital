...
import type { MenuItem, MenuOption } from '@/profiles/comida/comida-types'
import type { ComidaCategoryId } from '@/profiles/comida/comida-categories'
...
export function listMenu(opts: { category?: string; available?: boolean } = {}): Promise<{ items: MenuItem[] }> {
  ...
  const qs = p.toString()
  return apiFetch<{ items: MenuItem[] }>(`/api/comida/menu${qs ? `?${qs}` : ''}`)
}

export function getMenuItem(id: string): Promise<MenuItem> {
  return apiFetch<MenuItem>(`/api/comida/menu/${id}`)
}

export function createMenuItem(input: CreateMenuItemInput): Promise<MenuItem> {
  return apiFetch<MenuItem>('/api/comida/menu', { method: 'POST', body: JSON.stringify(input) })
}

export function updateMenuItem(id: string, input: UpdateMenuItemInput): Promise<MenuItem> {
  return apiFetch<MenuItem>(`/api/comida/menu/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}

export function toggleMenuItem(id: string, available: boolean): Promise<MenuItem> {
  return apiFetch<MenuItem>(`/api/comida/menu/${id}/toggle`, {
    method: 'PATCH',
...
export function deleteMenuItem(id: string): Promise<void> {
  return apiFetch<void>(`/api/comida/menu/${id}`, { method: 'DELETE' })
}
...
export function listOptions(itemId: string): Promise<{ items: MenuOption[] }> {
  return apiFetch<{ items: MenuOption[] }>(`/api/comida/menu/${itemId}/options`)
}

export function createOption(itemId: string, input: CreateOptionInput): Promise<MenuOption> {
  return apiFetch<MenuOption>(`/api/comida/menu/${itemId}/options`, {
    method: 'POST',
...
export function updateOption(itemId: string, optionId: string, input: UpdateOptionInput): Promise<MenuOption> {
  return apiFetch<MenuOption>(`/api/comida/menu/${itemId}/options/${optionId}`, {
    method: 'PATCH',
    body: JSON.stringify(input),
  })
}
...
export function toggleOption(itemId: string, optionId: string, available: boolean): Promise<MenuOption> {
  return apiFetch<MenuOption>(`/api/comida/menu/${itemId}/options/${optionId}/toggle`, {
    method: 'PATCH',
    body: JSON.stringify({ available }),
  })
}
...
export function deleteOption(itemId: string, optionId: string): Promise<void> {
  return apiFetch<void>(`/api/comida/menu/${itemId}/options/${optionId}`, { method: 'DELETE' })
}
