...
import type { Order, OrderStatus } from '@/profiles/comida/comida-types'
...
export function listOrders(
  ...
  const qs = p.toString()
  return apiFetch<OrderPage>(`/api/comida/orders${qs ? `?${qs}` : ''}`)
}

export function getOrder(id: string): Promise<Order> {
  return apiFetch<Order>(`/api/comida/orders/${id}`)
}
...
export function updateOrderStatus(
  ...
  return apiFetch<Order>(`/api/comida/orders/${id}/status`, {
    method: 'PATCH',
