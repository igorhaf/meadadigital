...
export type CreateProfessionalInput = { name: string; specialty?: string | null; notes?: string | null }
export type UpdateProfessionalInput = Partial<CreateProfessionalInput> & { active?: boolean }

export function listProfessionals(opts: { onlyActive?: boolean } = {}): Promise<{ items: AestheticProfessional[] }> {
...
export function createProfessional(input: CreateProfessionalInput): Promise<AestheticProfessional> {
  return apiFetch<AestheticProfessional>('/api/estetica/professionals', { method: 'POST', body: JSON.stringify(input) })
}

export function updateProfessional(id: string, input: UpdateProfessionalInput): Promise<AestheticProfessional> {
  return apiFetch<AestheticProfessional>(`/api/estetica/professionals/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}
