...
): Promise<{ items: AestheticProcedure[] }> {
...
export function createProcedure(input: CreateProcedureInput): Promise<AestheticProcedure> {
  return apiFetch<AestheticProcedure>('/api/estetica/procedures', {
    method: 'POST',
...
export function updateProcedure(
  ...
  return apiFetch<AestheticProcedure>(`/api/estetica/procedures/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(input),
  })
}
