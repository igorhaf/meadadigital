...
import {
  ...
  updateMenuItem,
  updateOption,
} from '@/lib/api/padaria/menu'
import { PADARIA_CATEGORIES, type PadariaCategoryId } from '@/profiles/padaria/padaria-categories'
import { formatBrl, type MenuItem, type MenuOption } from '@/profiles/padaria/padaria-types'
...
/**
 * Cardápio do PadariaBot. Itens agrupados por categoria, toggle de disponibilidade inline,
 * criação/edição via Modal, busca por nome. ESCAPADA (8.8): cada item pode ser SOB ENCOMENDA
 * (madeToOrder) com prazo próprio (leadTimeDays; vazio usa o default do config) e declarar
 * alérgenos (allergens). Modal secundário "Opções" (grupos Sabor/Recheio/Tamanho) com adicionar/
 * editar/remover/toggle — priceDeltaCents em R$. Preços salvos em centavos.
 */
export default function PadariaMenuPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['padaria-menu'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['padaria-menu'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Cardápio"
        description="Itens da sua padaria. A IA usa este cardápio (com as opções e os prazos) para atender os clientes."
                    ...
                    {catItems.map((it) => (
                      <div key={it.id} className="flex items-center justify-between gap-3 px-4 py-3">
                        <div className="min-w-0">
                          ...
                          {it.allergens && (
                            <p className="truncate text-xs text-muted-foreground">Alérgenos: {it.allergens}</p>
                          )}
                        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar item' : 'Novo item'} size="md">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.category}
                onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as PadariaCategoryId }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
                {PADARIA_CATEGORIES.map((c) => (
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Alérgenos (opcional)</label>
            <input value={form.allergens} onChange={(e) => setForm((f) => ({ ...f, allergens: e.target.value }))}
              maxLength={300} placeholder="Ex.: glúten, leite, ovos, amendoim"
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
          </div>
      ...
      {/* Modal secundário: opções/modifiers do item (Sabor/Recheio/Tamanho) */}
      <Modal
      ...
      >
        <div className="flex-1 min-w-[8rem]">
          ...
          <input value={form.groupLabel} onChange={(e) => setForm((f) => ({ ...f, groupLabel: e.target.value }))} required
            maxLength={80} placeholder="Sabor, Recheio, Tamanho…"
        ...
        <div className="flex-1 min-w-[8rem]">
          ...
          <input value={form.optionLabel} onChange={(e) => setForm((f) => ({ ...f, optionLabel: e.target.value }))} required
            maxLength={80} placeholder="Chocolate, Ninho, 2kg…"
            className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
        </div>
        <div className="w-28">
