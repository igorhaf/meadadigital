...
import {
  ...
  type OrderItem,
  type OrderStatus,
} from '@/profiles/suplementos/suplementos-types'

/** Resumo de um item com sua variante: "1× Whey Protein (Chocolate · 900g)". */
function itemLine(it: OrderItem): string {
  return `${it.qtd}× ${it.productName} (${it.variantLabel})`
}
...
function OrderCard({
  ...
  return (
    ...
    <Card className="space-y-2 p-3">
      ...
      <p className="text-xs text-muted-foreground">{order.deliveryAddress ?? 'Endereço não informado'}</p>
...
/**
 * Kanban de pedidos do SuplementosBot (loja de saúde / nutrição esportiva / varejo). Colunas:
 * Aguardando aceite → Em preparo → Saiu para entrega. Gate de aceite: em "aguardando" o lojista
 * Aceita (→ em_preparo) ou Recusa (Modal com motivo OPCIONAL → status recusado). Nas demais
 * colunas: Avançar / Cancelar (confirmado via AlertDialog). Entregues/recusados/cancelados ficam
 * na aba Histórico (recusado mostra o motivo). SÓ ENTREGA — cada card mostra o endereço; cada item
 * mostra a variante escolhida ("1× Whey (Chocolate · 900g)"). Atualiza a cada 30s.
 */
export default function SuplementosOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['suplementos-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['suplementos-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
