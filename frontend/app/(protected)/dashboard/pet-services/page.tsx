...
import { formatPrice, speciesLabel, type PetService, type PetSpecies } from '@/profiles/pet/pet-types'
...
const EMPTY: FormState = { name: '', category: '', durationMinutes: '45', price: '', speciesRestriction: '', description: '' }
                  ...
                  <div key={o.id} className="flex items-center justify-between gap-3 px-4 py-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {o.speciesRestriction && <Badge variant="info">{speciesLabel(o.speciesRestriction)}</Badge>}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar serviço' : 'Novo serviço'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Restrição de espécie</label>
              <select value={form.speciesRestriction}
                onChange={(e) => setForm((f) => ({ ...f, speciesRestriction: e.target.value as '' | PetSpecies }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
