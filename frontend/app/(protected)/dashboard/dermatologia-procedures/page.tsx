...
export default function DermatologiaProceduresPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      if (editing) {
        const prepCleared =
          form.prepInstructions.trim() === '' && (editing.prepInstructions ?? '') !== ''
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-xs text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
        A <strong>nota de preparo</strong> é enviada ao paciente{' '}
        <strong>exatamente como escrita</strong>, sem que a IA reescreva. Use-a só para orientações
        pré-procedimento (ex.: jejum, suspender ácido). Ela <strong>não é prontuário</strong> — não
        inclua diagnóstico, evolução ou dado clínico sensível.
      </div>
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                required
                maxLength={120}
                placeholder="Consulta, Botox, Limpeza de pele…"
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              A IA entrega este texto <strong>verbatim</strong> quando o paciente pedir o preparo da
              consulta — nunca o reescreve. <strong>Não é prontuário</strong>: sem diagnóstico ou
              dado clínico sensível.
            </p>
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">
