...
export default function PousadaReservationsPage() {
  ...
  const rooms = useQuery({
    queryKey: ['pousada-rooms-all'],
    queryFn: () => listRooms({ onlyActive: true }),
  })
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {r.roomName} · {formatDate(r.checkInDate)} → {formatDate(r.checkOutDate)} ·{' '}
                        {r.nights} noite(s) · {formatPrice(r.totalCents)}
                      </p>
                    </div>
                    <span className="text-xs text-muted-foreground">
                      {r.guestsCount} hóspede(s)
                    </span>
                  </button>
                ))}
              </div>
        ...
        >
          <div>
            ...
            >
              ...
              {(rooms.data?.items ?? []).map((r) => (
                <option key={r.id} value={r.id}>
                  {r.name} (cap. {r.capacity}, {formatPrice(r.nightlyRateCents)}/noite)
                </option>
              ))}
            </select>
          </div>
          ...
          {liveTotal != null && (
            <p className="rounded-md bg-muted/40 px-3 py-2 text-sm">
              {nights} noite(s) × {formatPrice(selectedRoom!.nightlyRateCents)} ={' '}
              <strong>{formatPrice(liveTotal)}</strong>
            </p>
          )}
          {formError && <p className="text-sm text-destructive">{formError}</p>}
          {conflict && (
            <p className="rounded-md bg-destructive/10 px-3 py-2 text-xs text-destructive">
              Ocupado por <strong>{conflict.guestName}</strong>, {formatDate(conflict.checkInDate)}{' '}
          ...
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  <dt className="text-xs text-muted-foreground">Quarto</dt>
                  <dd>{detail.roomName}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">Hóspedes</dt>
                  <dd>{detail.guestsCount}</dd>
                </div>
                <div>
                ...
                <div>
                  <dt className="text-xs text-muted-foreground">Noites</dt>
                  <dd>{detail.nights}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted-foreground">Total</dt>
                  <dd>{formatPrice(detail.totalCents)}</dd>
                </div>
                <div>
