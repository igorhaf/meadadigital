...
export default function AtelieArtisansPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return createArtisan({ name: form.name, specialty: form.specialty || null, notes: form.notes || null })
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={a.id} className="flex items-center justify-between gap-3 px-4 py-3">
              ...
              <div className="flex shrink-0 items-center gap-3">
                <label className="flex items-center gap-1 text-xs text-muted-foreground">
                  <input type="checkbox" checked={a.active} disabled={toggleMutation.isPending}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar artesão' : 'Novo artesão'} size="md">
