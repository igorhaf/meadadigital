...
type OpenForm = {
  customerName: string; consultantId: string; destination: string
  startDate: string; endDate: string; numTravelers: string; travelStyle: string
  briefing: string; notes: string
}
const EMPTY_OPEN: OpenForm = {
  customerName: '', consultantId: '', destination: '', startDate: '', endDate: '',
  numTravelers: '', travelStyle: '', briefing: '', notes: '',
}
...
export default function ViagensProposalsPage() {
  ...
  const openMutation = useMutation({
    mutationFn: () => openProposal({
      ...
      numTravelers: openForm.numTravelers ? Math.max(0, Math.round(Number(openForm.numTravelers))) : null,
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'inactive_consultant') setOpenError('Esse consultor está inativo.')
  ...
  const addDayMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['viagens-proposal', detailId] })
      setDayForm(EMPTY_DAY); setDayError(null)
    },
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'proposal_locked') setDayError('Esta proposta não aceita mais alteração do itinerário.')
  ...
  const reorderDayMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['viagens-proposal', detailId] }),
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'proposal_locked') setDayError('Esta proposta não aceita mais reordenação.')
  ...
  const statusMutation = useMutation({
    ...
    onError: (e) => {
      ...
      if (e instanceof ApiError && e.reason === 'empty_budget') setStatusError('Adicione ao menos um item de cotação antes de orçar.')
  ...
  return (
    <div className="space-y-6">
      ...
      {isError ? (
        ...
        <div className="divide-y divide-border rounded-lg border border-border">
          {items.map((prop: Proposal) => (
            <button key={prop.id} onClick={() => { setDetailId(prop.id); setItemError(null); setDayError(null); setStatusError(null) }}
              className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-muted/40">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{prop.customerName}</span>
                  {prop.destination && <span className="text-xs text-muted-foreground">{prop.destination}</span>}
      ...
      {/* Modal: nova proposta */}
      <Modal open={openModal} onClose={() => setOpenModal(false)} title="Nova proposta de viagem" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); openMutation.mutate() }}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Consultor (opcional)</label>
              <select value={openForm.consultantId} onChange={(e) => setOpenForm((f) => ({ ...f, consultantId: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
            ...
            <div>
              ...
              <input value={openForm.destination} onChange={(e) => setOpenForm((f) => ({ ...f, destination: e.target.value }))}
                placeholder="Paris, Cancún, Nordeste…"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Estilo de viagem</label>
              <input value={openForm.travelStyle} onChange={(e) => setOpenForm((f) => ({ ...f, travelStyle: e.target.value }))}
                placeholder="lua de mel, família, mochilão…"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Data de ida</label>
              <input type="date" value={openForm.startDate} onChange={(e) => setOpenForm((f) => ({ ...f, startDate: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Data de volta</label>
              <input type="date" value={openForm.endDate} onChange={(e) => setOpenForm((f) => ({ ...f, endDate: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Nº de viajantes</label>
              <input type="number" min="0" value={openForm.numTravelers} onChange={(e) => setOpenForm((f) => ({ ...f, numTravelers: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
          </div>
          <div>
            ...
            <textarea value={openForm.briefing} onChange={(e) => setOpenForm((f) => ({ ...f, briefing: e.target.value }))}
              rows={2} placeholder="O que o cliente imagina para a viagem"
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
          </div>
          <div>
      ...
      <Modal open={detailId !== null} onClose={() => { setDetailId(null); setStatusError(null); setItemError(null); setDayError(null) }}
        title="Proposta de viagem" size="lg">
        {detail.isPending || !p ? (
          <p className="text-sm text-muted-foreground">Carregando…</p>
        ) : (
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><dt className="text-xs text-muted-foreground">Consultor</dt><dd>{p.consultantName ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Destino</dt><dd>{p.destination ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Estilo</dt><dd>{p.travelStyle ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Período</dt><dd>{p.startDate ? formatDate(p.startDate) : '—'}{p.endDate ? ` – ${formatDate(p.endDate)}` : ''}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Viajantes</dt><dd>{p.numTravelers ?? '—'}</dd></div>
                ...
                <form className="flex flex-wrap items-end gap-2 rounded-lg border border-dashed border-border p-3"
                  ...
                  <div className="w-32">
                    ...
                    <select value={itemForm.category}
                      onChange={(e) => setItemForm((f) => ({ ...f, category: e.target.value as ItemCategoryId }))}
                  ...
                  <div className="flex-1 min-w-[8rem]">
                    ...
                    <input value={itemForm.description} onChange={(e) => setItemForm((f) => ({ ...f, description: e.target.value }))} required
                      maxLength={200} placeholder="Passagem GRU–CDG, hotel 4 noites…"
                      className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
                  </div>
                  <div className="w-16">
            ...
            <div className="space-y-2">
              ...
              {p.itinerary.length === 0 ? (
                ...
                <div className="divide-y divide-border rounded-lg border border-border">
                  {p.itinerary.map((day, idx) => (
                    <div key={day.id} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
                      <div className="min-w-0">
                        <span className="font-mono text-xs font-medium">Dia {day.dayNumber}</span>
                        {day.dayDate && <span className="ml-2 text-xs text-muted-foreground">{formatDate(day.dayDate)}</span>}
                      ...
                      {!locked && (
                        <div className="flex shrink-0 items-center gap-1">
                          <Button variant="outline" className="h-6 px-2 text-xs"
                            ...
                            onClick={() => moveDay(p.itinerary, idx, -1)}>↑</Button>
                          <Button variant="outline" className="h-6 px-2 text-xs"
                            disabled={idx === p.itinerary.length - 1 || reorderDayMutation.isPending}
                            onClick={() => moveDay(p.itinerary, idx, 1)}>↓</Button>
                          <Button variant="outline" className="h-6 px-2 text-xs"
                            disabled={deleteDayMutation.isPending} onClick={() => deleteDayMutation.mutate(day.id)}>
                            Remover
                          </Button>
                        </div>
                      )}
              ...
              {!locked && (
                <form className="flex flex-wrap items-end gap-2 rounded-lg border border-dashed border-border p-3"
                  onSubmit={(e) => { e.preventDefault(); addDayMutation.mutate() }}>
                  <div className="w-36">
                    <label className="mb-1 block text-xs font-medium text-muted-foreground">Data (opcional)</label>
                    <input type="date" value={dayForm.dayDate}
                      onChange={(e) => setDayForm((f) => ({ ...f, dayDate: e.target.value }))}
                      className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
                  </div>
                  <div className="flex-1 min-w-[8rem]">
                    <label className="mb-1 block text-xs font-medium text-muted-foreground">Título do dia</label>
                    <input value={dayForm.title} onChange={(e) => setDayForm((f) => ({ ...f, title: e.target.value }))} required
                      maxLength={200} placeholder="Chegada em Paris, City tour, Versalhes…"
                      className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
                  </div>
                  <div className="flex-1 min-w-[8rem]">
                    <label className="mb-1 block text-xs font-medium text-muted-foreground">Descrição (opcional)</label>
                    <input value={dayForm.description} onChange={(e) => setDayForm((f) => ({ ...f, description: e.target.value }))}
                      placeholder="O que está previsto no dia"
                      className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
                  </div>
                  <Button type="submit" className="h-8 px-3 text-xs" disabled={addDayMutation.isPending}>Adicionar</Button>
                </form>
              )}
