...
type FormState = { name: string; category: string; durationMinutes: string; price: string; notes: string }
...
export default function EsteticaProceduresPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'procedure_in_use') {
        alert('Este procedimento está em agendamentos ou pacotes — não pode ser excluído. Desative-o.')
      }
    },
  })
  ...
  function openEdit(p: AestheticProcedure) {
    setEditing(p)
    setForm({
      name: p.name, category: p.category ?? '', durationMinutes: String(p.durationMinutes),
      price: (p.unitPriceCents / 100).toFixed(2), notes: p.notes ?? '',
    })
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar procedimento' : 'Novo procedimento'} size="md">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}
                placeholder="facial…" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Duração (min)</label>
              <input type="number" min="15" value={form.durationMinutes} required
                onChange={(e) => setForm((f) => ({ ...f, durationMinutes: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Preço/sessão (R$)</label>
