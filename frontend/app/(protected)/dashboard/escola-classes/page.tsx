...
const EMPTY: FormState = {
  name: '', grade: '', shift: 'manha', capacity: '20', monthly: '', year: String(new Date().getFullYear()), description: '',
}
...
export default function EscolaClassesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar turma' : 'Nova turma'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={form.grade} onChange={(e) => setForm((f) => ({ ...f, grade: e.target.value }))} required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.shift} onChange={(e) => setForm((f) => ({ ...f, shift: e.target.value as EscolaShift }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
            ...
            <div>
              ...
              <input type="number" min="1" max="200" value={form.capacity} required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Mensalidade (R$)</label>
              <input type="number" min="0" step="0.01" value={form.monthly} required
                onChange={(e) => setForm((f) => ({ ...f, monthly: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Ano letivo</label>
