...
import { listOrders, updateOrderStatus } from '@/lib/api/comida/orders'
...
import {
  ...
  type OrderItem,
  type OrderStatus,
} from '@/profiles/comida/comida-types'
...
/**
 * Kanban de pedidos do ComidaBot (delivery). Colunas: Aguardando aceite → Em preparo → Saiu pra
 ...
 * Atualiza a cada 30s.
 */
export default function ComidaOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['comida-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['comida-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['comida-orders'] }),
  })
  ...
  const dnd = useKanbanDnd({
    canDrop: (id, target) => {
      ...
      // 'aguardando' avança via "Aceitar" (NEXT_STATUS já aponta o destino); demais via NEXT_STATUS.
