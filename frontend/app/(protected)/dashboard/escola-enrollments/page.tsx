...
export default function EscolaEnrollmentsPage() {
  ...
  const classes = useQuery({ queryKey: ['escola-classes-all'], queryFn: () => listClasses({ onlyActive: true }) })
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => {
      ...
      } else if (e instanceof ApiError && (e.reason === 'class_inactive' || e.reason === 'student_inactive')) {
            ...
            <button key={m.id} onClick={() => setDetail(m)}
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {m.className} · {m.classGrade} · {shiftLabel(m.classShift)} · desde {formatDate(m.startDate)}
                </p>
              </div>
              <span className="text-xs text-muted-foreground">{formatBrl(m.classMonthlyCents)}/mês</span>
            </button>
          ))}
        </div>
      )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova matrícula" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          <div>
            ...
            <select value={form.studentId} onChange={(e) => setForm((f) => ({ ...f, studentId: e.target.value }))} required
              ...
              {(students.data?.items ?? []).map((s) => (
                <option key={s.id} value={s.id}>{s.name}{s.intendedGrade ? ` (${s.intendedGrade})` : ''}</option>
              ))}
            </select>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={createMutation.isPending || !form.studentId || !form.classId}>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                <div><dt className="text-xs text-muted-foreground">Turma</dt><dd>{detail.className} ({formatBrl(detail.classMonthlyCents)}/mês)</dd></div>
                <div><dt className="text-xs text-muted-foreground">Série / turno</dt><dd>{detail.classGrade} · {shiftLabel(detail.classShift)}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Responsável</dt><dd>{detail.responsibleName ?? '—'}</dd></div>
