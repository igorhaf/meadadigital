...
export default function AcademiaReferralsPage() {
  ...
  function confirmConvert(r: Referral) {
    if (confirm(`Marcar a indicação de ${r.referredName} como convertida?`))
      convertMutation.mutate(r.id)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as '' | ReferralStatusFilter)}
              ...
              {referrals.map((r) => (
                <tr key={r.id} className="border-b border-border last:border-b-0">
                  ...
                  <td className="px-3 py-2">
                    ...
                    {r.referredPhone && (
                      <span className="ml-2 text-xs text-muted-foreground">{r.referredPhone}</span>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    {r.referrerContactId
                      ? (referrerNameByContact.get(r.referrerContactId) ?? '—')
                      : '—'}
                  </td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova indicação" size="md">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            O código da indicação é gerado automaticamente.
          </p>
