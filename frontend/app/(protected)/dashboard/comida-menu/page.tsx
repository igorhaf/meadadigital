...
import {
  ...
  updateMenuItem,
  updateOption,
} from '@/lib/api/comida/menu'
import { COMIDA_CATEGORIES, type ComidaCategoryId } from '@/profiles/comida/comida-categories'
import { formatBrl, type MenuItem, type MenuOption } from '@/profiles/comida/comida-types'
...
const EMPTY_FORM: FormState = { name: '', description: '', price: '', category: 'lanches' }
...
/**
 * Cardápio do ComidaBot (delivery iFood-style). Itens agrupados por categoria, toggle de
 ...
 */
export default function ComidaMenuPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['comida-menu'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['comida-menu'] }),
  })
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.category}
                onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as ComidaCategoryId }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
                {COMIDA_CATEGORIES.map((c) => (
