...
import { listOrders, updateOrderStatus } from '@/lib/api/las/orders'
...
import {
  ...
  type OrderItem,
  type OrderStatus,
} from '@/profiles/las/las-types'

/** Resumo de um item com sua variante: "3× Lã Merino (Azul / lote L2024-A)". */
function itemLine(it: OrderItem): string {
  return `${it.qtd}× ${it.productName} (${it.color} / lote ${it.dyeLot})`
}
...
/**
 * Kanban de pedidos do LasBot (loja de lãs / novelos / tricô-crochê — varejo). Colunas: Aguardando
 * aceite → Separando → Enviado. Gate de aceite: em "aguardando" o lojista Aceita (→ separando) ou
 * Recusa (Modal com motivo OPCIONAL → status recusado). Nas demais colunas: Avançar / Cancelar
 * (confirmado via AlertDialog). Entregues/recusados/cancelados ficam na aba Histórico (recusado
 * mostra o motivo). Cada item mostra a variante escolhida ("3× Lã Merino (Azul / lote L2024-A)") e
 * o card exibe a forma de entrega (entrega/retirada) com endereço + o selo "mesmo lote garantido"
 * quando o pedido pediu garantia de mesmo lote de tingimento. Atualiza a cada 30s.
 */
export default function LasOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['las-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['las-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['las-orders'] }),
  })
  ...
  // (NEXT_STATUS do status atual). Recusar/cancelar continuam por botão (exigem motivo/confirmação).
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Pedidos"
        description="Aceite ou recuse novos pedidos e acompanhe a separação e o envio."
      />
              ...
              {historyItems.map((o) => (
                <div key={o.id} className="flex flex-col gap-1 px-4 py-3 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {o.sameLotGuaranteed && <Badge variant="info">mesmo lote</Badge>}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={300}
              placeholder="Ex.: lote sem estoque, fora da área de entrega…"
