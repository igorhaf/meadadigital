...
const EMPTY: FormState = { professionalId: '', patientId: '', procedureTypeId: '', date: '', time: '', notes: '' }

function groupByDay(items: DermatologiaAppointment[]): { day: string; items: DermatologiaAppointment[] }[] {
...
export default function DermatologiaAppointmentsPage() {
  ...
  const professionals = useQuery({ queryKey: ['dermatologia-appt-professionals'], queryFn: () => listProfessionals({ onlyActive: true }) })
  const patients = useQuery({ queryKey: ['dermatologia-appt-patients'], queryFn: () => listPatients({ active: true }) })
  const procedureTypes = useQuery({ queryKey: ['dermatologia-appt-types'], queryFn: () => listProcedureTypes({ onlyActive: true }) })
    ...
    <div className="space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova consulta" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Tipo de atendimento</label>
            <select value={form.procedureTypeId}
              onChange={(e) => setForm((f) => ({ ...f, procedureTypeId: e.target.value }))} required
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
              <option value="">Selecione…</option>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><dt className="text-xs text-muted-foreground">Tipo</dt><dd>{detail.procedureTypeName}</dd></div>
