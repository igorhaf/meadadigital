...
import {
  ...
  listPatients,
  updatePatient,
} from '@/lib/api/dermatologia/patients'
import { getMyContacts } from '@/lib/supabase/contacts'
import { formatDate, type DermatologiaPatient } from '@/profiles/dermatologia/dermatologia-types'
...
const EMPTY: FormState = { contactId: '', name: '', birthDate: '', notes: '' }

/**
 * Pacientes (camada 8.11). SUB-ENTIDADE do contato (cliente). Lista com busca por nome /
 ...
 */
export default function DermatologiaPatientsPage() {
  const qc = useQueryClient()
  const [search, setSearch] = useState<string>('')
  const [showInactive, setShowInactive] = useState(false)

  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<DermatologiaPatient | null>(null)
  const [form, setForm] = useState<FormState>(EMPTY)
  const [formError, setFormError] = useState<string | null>(null)

  const { data, isPending, isError } = useQuery({
    queryKey: ['dermatologia-patients', search, showInactive],
  ...
  const contacts = useQuery({ queryKey: ['dermatologia-contacts'], queryFn: () => getMyContacts() })
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['dermatologia-patients'] })
  ...
  const archiveMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['dermatologia-patients'] }),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'patient_in_use') {
        alert('Este paciente tem consultas — não pode ser excluído. Arquive-o.')
      }
    },
  })

  function openCreate() { setEditing(null); setForm(EMPTY); setFormError(null); setModalOpen(true) }
  function openEdit(p: DermatologiaPatient) {
    setEditing(p)
    setForm({
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
              rows={2} placeholder="administrativo — sem dado clínico (LGPD)"
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
          </div>
