...
function StatusBadge({ status }: { status: AestheticPackageStatusId }) {
  const variant =
    ...
    : status === 'esgotado' ? 'info'
    : 'muted'
  return <Badge variant={variant}>{statusLabel(status)}</Badge>
}

type CreateForm = { customerName: string; procedureId: string; totalSessions: string; notes: string }
const EMPTY_CREATE: CreateForm = { customerName: '', procedureId: '', totalSessions: '10', notes: '' }
...
export default function EsteticaPackagesPage() {
  ...
  const [statusTarget, setStatusTarget] = useState<{ id: string; next: AestheticPackageStatusId } | null>(null)
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, next }: { id: string; next: AestheticPackageStatusId }) => updatePackageStatus(id, next),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['estetica-packages'] })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setCreateForm(EMPTY_CREATE); setCreateError(null); setCreateOpen(true) }}>Novo pacote</Button>}
      />
              ...
              <div key={pk.id} className="flex items-center justify-between gap-3 px-4 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {pk.sessionsRemaining} de {pk.totalSessions} sessões restantes · {formatPrice(pk.totalCents)} · {formatDate(pk.purchasedAt)}
                  </p>
                </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} pacotes</span>
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          <div>
            ...
            <input value={createForm.customerName} onChange={(e) => setCreateForm((f) => ({ ...f, customerName: e.target.value }))}
              placeholder="Nome do cliente" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
          </div>
            ...
            <div>
              ...
              <select value={createForm.procedureId} onChange={(e) => setCreateForm((f) => ({ ...f, procedureId: e.target.value }))} required
                ...
                {(procedures.data?.items ?? []).map((p) => (
                  <option key={p.id} value={p.id}>{p.name} · {formatPrice(p.unitPriceCents)}/sessão</option>
                ))}
              </select>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Nº de sessões</label>
              <input type="number" min="1" value={createForm.totalSessions} required
          ...
          <p className="text-xs text-muted-foreground">O total é calculado automaticamente (sessões × preço do procedimento). O pacote nasce pendente — ative-o ao confirmar o pagamento.</p>
          {createError && <p className="text-sm text-destructive">{createError}</p>}
          <div className="flex justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => setCreateOpen(false)}>Cancelar</Button>
            <Button type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? 'Criando…' : 'Criar pacote'}</Button>
          </div>
        </form>
      </Modal>
