...
import {
  ...
  togglePlanner,
  updatePlanner,
} from '@/lib/api/casamento/planners'
...
/**
 * Assessores do CasamentoBot (camada 8.7). CRUD via Modal, toggle ativo, exclusão protegida (409
 * planner_in_use → orienta a desativar). Catálogo simples — sem agenda.
 */
export default function CasamentoPlannersPage() {
  const qc = useQueryClient()
  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<WeddingPlanner | null>(null)
  const [form, setForm] = useState<FormState>(EMPTY)
  const [formError, setFormError] = useState<string | null>(null)

  const { data, isPending, isError } = useQuery({
    queryKey: ['casamento-planners'],
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['casamento-planners'] })
      setModalOpen(false); setEditing(null); setForm(EMPTY); setFormError(null)
    },
    onError: () => setFormError('Erro ao salvar o assessor.'),
  })

  const toggleMutation = useMutation({
    mutationFn: (p: WeddingPlanner) => togglePlanner(p.id, !p.active),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['casamento-planners'] }),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'planner_in_use') {
        alert('Este assessor está atribuído a propostas — não pode ser excluído. Desative-o.')
      }
    },
  })

  function openCreate() { setEditing(null); setForm(EMPTY); setFormError(null); setModalOpen(true) }
  function openEdit(p: WeddingPlanner) {
    setEditing(p)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Assessores"
        description="Equipe responsável pelos casamentos. Atribuir um assessor a uma proposta é opcional."
        actions={<Button onClick={openCreate}>Novo assessor</Button>}
      />

      {isError ? (
        <p className="text-sm text-destructive">Erro ao carregar os assessores.</p>
      ) : isPending ? (
        <p className="text-sm text-muted-foreground">Carregando…</p>
      ) : items.length === 0 ? (
        <p className="text-sm text-muted-foreground">Nenhum assessor cadastrado ainda.</p>
      ) : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar assessor' : 'Novo assessor'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            ...
            <input value={form.specialty} onChange={(e) => setForm((f) => ({ ...f, specialty: e.target.value }))}
              placeholder="cerimônia, decoração, festa…"
