...
export default function CasesPage() {
  ...
  const { data, isPending, isError } = useQuery({
    ...
    queryFn: () =>
      listCases({ status: status || undefined, search: search || undefined, page, pageSize: 20 }),
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'invalid_cnj')
        setFormError('CNJ inválido (verifique o dígito verificador).')
      else if (e instanceof ApiError && e.reason === 'duplicate_cnj')
        setFormError('Já existe um processo com este CNJ.')
    ...
    <div className="space-y-6">
            ...
            <Link key={c.id} href={`/dashboard/cases/${c.id}`}>
              <Card className="space-y-1 p-4 transition-colors hover:bg-muted/40">
                ...
                <p className="text-xs text-muted-foreground">
                  {c.updatesCount} andamento(s) · atualizado{' '}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>
            Página {page + 1} de {totalPages} · {total} processo(s)
          </span>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo processo" size="md">
        ...
        >
          <div>
            ...
            <select
              value={form.legalClientId}
              onChange={(e) => setForm((f) => ({ ...f, legalClientId: e.target.value }))}
              required
          ...
          <div>
            ...
            <input
              value={form.cnj}
              onChange={(e) => setForm((f) => ({ ...f, cnj: formatCnj(e.target.value) }))}
              required
