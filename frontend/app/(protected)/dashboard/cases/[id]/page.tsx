...
import { addUpdate, deleteUpdate, getCase, updateCaseStatus } from '@/lib/api/legal/cases'
import {
  LEGAL_CASE_STATUSES,
  statusLabel,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <select
            value=""
            onChange={(e) => {
              if (e.target.value) setStatusTarget(e.target.value as LegalCaseStatusId)
            }}
      ...
      <Card>
        <Section title="Detalhes">
          <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
            <div>
              <dt className="text-xs text-muted-foreground">Vara</dt>
              <dd>{c.court ?? '—'}</dd>
            </div>
            <div>
              <dt className="text-xs text-muted-foreground">Fórum</dt>
              <dd>{c.forum ?? '—'}</dd>
            </div>
            <div>
      ...
      <AlertDialog
        ...
        onConfirm={() => {
          if (deleteTarget) deleteUpdateMutation.mutate(deleteTarget.id)
        }}
      />
