...
/**
 * Acesso a {@code cursos_coupons} — subclasse FINA do motor comum {@link CouponRepositoryBase}
 * (unificação 2026-07 dos 7 clones de cupom; SQL e semântica vivem na base).
 */
@Repository
public class CursosCouponRepository extends CouponRepositoryBase<CursosCoupon> {

    private static final RowMapper<CursosCoupon> MAPPER = (rs, rn) -> {
        Date vu = rs.getDate("valid_until");
        Object maxUses = rs.getObject("max_uses");
        return new CursosCoupon(
    ...
    public CursosCouponRepository(JdbcTemplate jdbcTemplate) {
        super(jdbcTemplate);
    }

    @Override
    protected String table() {
        return "cursos_coupons";
    }
    ...
    @Override
    protected RowMapper<CursosCoupon> mapper() {
        return MAPPER;
    }
}
