...
/**
 * Cupons de desconto do tenant cursos (camada 8.20, backlog #1 — clone do chassi sushi). TENANT +
 * perfil 'cursos' only. CRUD do catálogo de cupons; a aplicação acontece na criação do pedido. Sob
 * {@code /api/cursos/**}.
 */
@RestController
public class CursosCouponController {

    private final CursosCouponService service;
    private final CursosProfileGuard profileGuard;

    public CursosCouponController(CursosCouponService service, CursosProfileGuard profileGuard) {
    ...
    @GetMapping("/api/cursos/coupons/{id}")
    ...
    @PostMapping("/api/cursos/coupons")
    ...
    @PatchMapping("/api/cursos/coupons/{id}")
    ...
    @PatchMapping("/api/cursos/coupons/{id}/toggle")
    ...
    @DeleteMapping("/api/cursos/coupons/{id}")
