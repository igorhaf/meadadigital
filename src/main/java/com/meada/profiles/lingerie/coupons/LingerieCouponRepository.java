...
/**
 * Acesso a {@code lingerie_coupons} — subclasse FINA do motor comum {@link CouponRepositoryBase}
 * (unificação 2026-07 dos 7 clones de cupom; SQL e semântica vivem na base).
 */
@Repository
public class LingerieCouponRepository extends CouponRepositoryBase<LingerieCoupon> {

    private static final RowMapper<LingerieCoupon> MAPPER = (rs, rn) -> {
        Date vu = rs.getDate("valid_until");
        Object maxUses = rs.getObject("max_uses");
        return new LingerieCoupon(
    ...
    public LingerieCouponRepository(JdbcTemplate jdbcTemplate) {
        super(jdbcTemplate);
    }

    @Override
    protected String table() {
        return "lingerie_coupons";
    }
    ...
    @Override
    protected RowMapper<LingerieCoupon> mapper() {
        return MAPPER;
    }
}
