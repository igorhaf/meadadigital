...
/**
 * Acesso a {@code pizzaria_coupons} — subclasse FINA do motor comum {@link CouponRepositoryBase}
 * (unificação 2026-07 dos 7 clones de cupom; SQL e semântica vivem na base).
 */
@Repository
public class PizzariaCouponRepository extends CouponRepositoryBase<PizzariaCoupon> {

    private static final RowMapper<PizzariaCoupon> MAPPER = (rs, rn) -> {
        Date vu = rs.getDate("valid_until");
        Object maxUses = rs.getObject("max_uses");
        return new PizzariaCoupon(
    ...
    public PizzariaCouponRepository(JdbcTemplate jdbcTemplate) {
        super(jdbcTemplate);
    }

    @Override
    protected String table() {
        return "pizzaria_coupons";
    }
    ...
    @Override
    protected RowMapper<PizzariaCoupon> mapper() {
        return MAPPER;
    }
}
