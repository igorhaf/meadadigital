...
/**
 * Config de fidelidade do tenant pizzaria (camada 8.9, backlog #2 — clone do chassi sushi). TENANT +
 * perfil 'pizzaria' only. GET (com fallback p/ defaults) + PUT (upsert). Sob {@code /api/pizzaria/**}.
 */
@RestController
public class PizzariaLoyaltyConfigController {

    private final PizzariaLoyaltyConfigService service;
    private final PizzariaProfileGuard profileGuard;

    public PizzariaLoyaltyConfigController(PizzariaLoyaltyConfigService service, PizzariaProfileGuard profileGuard) {
    ...
    @GetMapping("/api/pizzaria/loyalty")
    ...
    @PutMapping("/api/pizzaria/loyalty")
