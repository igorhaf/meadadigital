package com.meada.profiles.modainfantil.alerts;
 ...
 */
@Service
public class ModaInfantilStockAlertService {

    private static final Logger log = LoggerFactory.getLogger(ModaInfantilStockAlertService.class);

    private final JdbcTemplate jdbcTemplate;
    private final com.meada.profiles.modainfantil.orders.ModaInfantilOrderNotifier notifier;

    public ModaInfantilStockAlertService(JdbcTemplate jdbcTemplate,
                                         com.meada.profiles.modainfantil.orders.ModaInfantilOrderNotifier notifier) {
    ...
    public boolean register(UUID companyId, UUID contactId, UUID variantId) {
        Integer ok = jdbcTemplate.query(
                "select 1 from moda_infantil_variants v join moda_infantil_products p on p.id = v.product_id "
        ...
        int n = jdbcTemplate.update(
            "insert into moda_infantil_stock_alerts (company_id, contact_id, variant_id) "
    ...
    public int notifyBackInStock(UUID companyId, UUID variantId) {
        ...
        List<Pending> pendings = jdbcTemplate.query(
            "select a.id as alert_id, "
                ...
                + "from moda_infantil_stock_alerts a "
                + "join contacts ct on ct.id = a.contact_id "
                + "join moda_infantil_variants v on v.id = a.variant_id "
                + "join moda_infantil_products p on p.id = v.product_id "
        ...
        for (Pending p : pendings) {
            try {
                ...
                jdbcTemplate.update(
                    "update moda_infantil_stock_alerts set notified_at = now() where id = ?", p.alertId());
                sent++;
            } catch (Exception e) {
                log.warn("moda-alert: failed alert {} ({})", p.alertId(), e.getMessage());
            }
        }
        return sent;
    }
