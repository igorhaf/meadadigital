...
/**
 * Cupons de desconto do tenant las (camada 8.9, backlog #1 — clone do chassi sushi). TENANT +
 * perfil 'las' only. CRUD do catálogo de cupons; a aplicação acontece na criação do pedido. Sob
 * {@code /api/las/**}.
 */
@RestController
public class LasCouponController {

    private final LasCouponService service;
    private final LasProfileGuard profileGuard;

    public LasCouponController(LasCouponService service, LasProfileGuard profileGuard) {
    ...
    @PatchMapping("/api/las/coupons/{id}")
    ...
    @PatchMapping("/api/las/coupons/{id}/toggle")
    ...
    @DeleteMapping("/api/las/coupons/{id}")
