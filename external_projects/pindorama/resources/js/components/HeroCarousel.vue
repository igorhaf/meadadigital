...
<template>
    <div class="relative overflow-hidden rounded-3xl">
        ...
        >
            <Transition enter-active-class="transition duration-700" enter-from-class="opacity-0" leave-active-class="transition duration-700" leave-to-class="opacity-0">
                <img v-if="active.image" :key="`img-${current}`" :src="active.image" alt="" class="absolute inset-0 h-full w-full object-cover" />
            </Transition>
            <div v-if="active.image" class="absolute inset-0" :style="{ backgroundImage: `linear-gradient(100deg, ${active.from}f2 0%, ${active.from}cc 38%, ${active.from}55 68%, transparent 95%)` }"></div>
            <Transition mode="out-in" enter-active-class="transition duration-500" enter-from-class="opacity-0 translate-y-3" leave-active-class="transition duration-200" leave-to-class="opacity-0">
                <div :key="current" class="relative z-10 max-w-xl">
                    <span class="mb-3 inline-block rounded-full bg-white/20 px-3 py-1 text-xs font-semibold uppercase tracking-wide backdrop-blur">Bem-estar em destaque</span>
                    ...
                    <a v-if="active.link" :href="active.link" class="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-bold text-brand-800 shadow-lg transition hover:bg-neutral-100">
